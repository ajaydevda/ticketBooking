...
abstract class AppRepo1 {
  Future<dynamic> postApiResponse({required jsonPostData, required api});

  Future<dynamic> putApiResponse({required jsonPostData, required api});

  Future<dynamic> getApiResponse({required jsonPostData, required api});

  Future<dynamic> postApiResponseWithBearerToken(
      {required jsonPostData, required api});
}
...
class AppRepo extends AppRepo1 {
  ...
  Future<dynamic> putApiResponse({jsonPostData, api}) async {
    ...
    var response = await http.put(Uri.parse(api),
        body: jsonPostData, headers: {"Content-Type": "application/json"});
  ...
  @override
  Future<dynamic> postApiResponseWithBearerToken({jsonPostData, api}) async {
    try {
      print("====url===>" + api);
      print("====request===>" + jsonPostData);

      var response =
          await http.post(Uri.parse(api), body: jsonPostData, headers: {
